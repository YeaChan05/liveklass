...
const SCENARIO_MAX_DURATION = __ENV.SCENARIO_MAX_DURATION || '5m';
...
const FIRST_REFILL_APPLICANT_COUNT = Number(__ENV.FIRST_REFILL_APPLICANT_COUNT || EXPIRE_PENDING_COUNT);
const SECOND_REFILL_APPLICANT_COUNT = Number(__ENV.SECOND_REFILL_APPLICANT_COUNT || EXPIRE_PENDING_COUNT);
const VUS = Number(__ENV.VUS || 100);

const WAIT_BEFORE_FIRST_REFILL_SECONDS = Number(__ENV.WAIT_BEFORE_FIRST_REFILL_SECONDS || 20);
const WAIT_BETWEEN_REFILL_WAVES_SECONDS = Number(__ENV.WAIT_BETWEEN_REFILL_WAVES_SECONDS || 70);
const FIRST_REFILL_POLL_TIMEOUT_SECONDS = Number(__ENV.FIRST_REFILL_POLL_TIMEOUT_SECONDS || 30);
const SECOND_REFILL_POLL_TIMEOUT_SECONDS = Number(__ENV.SECOND_REFILL_POLL_TIMEOUT_SECONDS || 30);
...
const firstRefillReady = new Counter('first_refill_ready');
const firstRefillExpired = new Counter('first_refill_expired');
const secondRefillReady = new Counter('second_refill_ready');
const secondRefillConfirmed = new Counter('second_refill_confirmed');
...
export const options = {
  ...
  scenarios: {
    concurrent_double_expiration_refill_to_one_course: {
      executor: 'shared-iterations',
      vus: VUS,
      iterations: FIRST_REFILL_APPLICANT_COUNT + SECOND_REFILL_APPLICANT_COUNT,
  ...
  thresholds: {
    ...
    first_refill_ready: [`count==${FIRST_REFILL_APPLICANT_COUNT}`],
    first_refill_expired: [`count==${FIRST_REFILL_APPLICANT_COUNT}`],
    second_refill_ready: [`count==${SECOND_REFILL_APPLICANT_COUNT}`],
    second_refill_confirmed: [`count==${SECOND_REFILL_APPLICANT_COUNT}`],
...
export function setup() {
  ...
  console.log(
      `waiting before first refill. seconds=${WAIT_BEFORE_FIRST_REFILL_SECONDS}, ` +
      `unconfirmedPending=${EXPIRE_PENDING_COUNT}, firstRefillApplicants=${FIRST_REFILL_APPLICANT_COUNT}, ` +
      `secondRefillApplicants=${SECOND_REFILL_APPLICANT_COUNT}`,
  );
...
export default function (data) {
  ...
  if (index < FIRST_REFILL_APPLICANT_COUNT) {
    handleFirstRefillWave(data.courseId, index);
    return;
  }

  handleSecondRefillWave(data.courseId, index - FIRST_REFILL_APPLICANT_COUNT);
}
...
function handleFirstRefillWave(courseId, waveIndex) {
  ...
  const enrollment = waitForRefillPendingEnrollment(token, courseId, waveIndex, FIRST_REFILL_POLL_TIMEOUT_SECONDS, 'first');
  firstRefillReady.add(1);

  sleep(WAIT_BETWEEN_REFILL_WAVES_SECONDS);
  ...
  firstRefillExpired.add(1);
  check(enrollment, {
    '1차 재신청 대기열 진입 성공': (body) => body && body.enrollmentId != null,
  });
}

function handleSecondRefillWave(courseId, waveIndex) {
  const tokenIndex = COURSE_CAPACITY + FIRST_REFILL_APPLICANT_COUNT + waveIndex;
  const token = applicantTokens[tokenIndex];
  ...
  const enrollment = waitForRefillPendingEnrollment(token, courseId, waveIndex, SECOND_REFILL_POLL_TIMEOUT_SECONDS, 'second');
  secondRefillReady.add(1);

  const confirmRes = confirmEnrollment(token, enrollment.enrollmentId);
  const confirmBody = parseJsonOrFail(confirmRes, `second refill confirm index=${waveIndex}`);
  const confirmed = isConfirmedEnrollment(confirmRes, confirmBody);

  if (confirmed) {
    secondRefillConfirmed.add(1);
  } else {
    scenarioFailed.add(1);
    console.error(`second refill confirm failed. index=${waveIndex}`);
  ...
  check(confirmRes, {
    '2차 대기자 승격 후 결제 확정 성공': () => confirmed,
  });
}
...
function assertApplicantTokensValidLongEnough() {
  const requiredTtlSeconds =
      ...
      WAIT_BETWEEN_REFILL_WAVES_SECONDS +
      Math.max(FIRST_REFILL_POLL_TIMEOUT_SECONDS, SECOND_REFILL_POLL_TIMEOUT_SECONDS) +
      120;
  const tokenIndexes = [
    0,
    COURSE_CAPACITY - 1,
    COURSE_CAPACITY,
    COURSE_CAPACITY + FIRST_REFILL_APPLICANT_COUNT + SECOND_REFILL_APPLICANT_COUNT - 1,
  ];
...
function validateConfiguration() {
  ...
  if (FIRST_REFILL_APPLICANT_COUNT + SECOND_REFILL_APPLICANT_COUNT > APPLICANT_COUNT - COURSE_CAPACITY) {
    fail(
        `FIRST_REFILL_APPLICANT_COUNT + SECOND_REFILL_APPLICANT_COUNT must be less than or equal to waitlisted count. ` +
        `firstRefillApplicantCount=${FIRST_REFILL_APPLICANT_COUNT}, ` +
        `secondRefillApplicantCount=${SECOND_REFILL_APPLICANT_COUNT}, ` +
