...
- For behavior-changing work, follow `docs/WORKFLOW.md`: analyze requirements first, write unit and integration tests before production code, confirm RED before implementation, then GREEN and refactor.
- Before code changes, read `docs/RULE.md` required references (`docs/rule/architecture_guide.puml`, `docs/rule/module`, `dependencies`, `code_convention`) and respect module boundaries and dependency direction.
...
- Verify with the narrowest relevant tests first, then lint/typecheck/static analysis; run `./gradlew jacocoRootReport` when coverage can be affected. Final reports include changed files, simplifications, and remaining risks.
...
| `executor` | `gpt-5.4-mini` | medium | Code implementation, refactoring, feature work (deep-worker, standard) |
