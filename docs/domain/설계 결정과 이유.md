...
사용자가 자신의 수강 신청 내역을 조회할 때는 `CONFIRMED`, `CANCELLED`만 반환합니다.  
`PENDING`은 아직 확정된 이력이 아니라 현재 결제 대기 상태이고, `EXPIRED`는 내부 만료 상태이므로 내역 조회에서는 제외합니다.

확정 신청과 취소 신청은 별도 이력 테이블을 만들지 않고 `enrollments` 테이블의 상태로 저장합니다.  
테이블 간 FK는 만들지 않으며, `course_id`, `member_id` 같은 식별자 컬럼만 전달합니다.
내역 조회는 `member_id`, `status` 기준으로 조회하므로 해당 조합에 맞는 인덱스를 둡니다.
